...
//    mysql_select_db('') or die ('Negaliu pasirinkti duomenu baze: ' . mysql_error() );
    ...
    //  
    @mysqli_query ('insert into CSV_load (data, userid) values(now(), "'.$session->username.'")');
    ...
    {
    mysql_query ('INSERT INTO prad_d (`Ken`, `Lie_d`, `Lie_z`, `Lan`, `Nam`, `War`, `Men`,`Sst`,load_id, userid) VALUES ("'.$matica_gera[$i]["Ken"].'", "'.$matica_gera[$i]["Lie_d"].'","'.$matica_gera[$i]["Lie_z"].'", "'.$matica_gera[$i]["Lan"].'","'. $matica_gera[$i]["Nam"].'", "'.$matica_gera[$i]["War"].'", "'.str_replace(',','.',$matica_gera[$i]["Men"]).'", "'.$matica_gera[$i]["Sst"].'",'.$load_id.', "'.$session->username.'");');
    }
